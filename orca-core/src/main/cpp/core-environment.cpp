...
jobject environment::getApplicationContext(jobject context) {
    ...
    if (application_clz != NULL) {
        ...
        if (superClass != nullptr) {
            jmethodID getNameMethod = jniEnv->GetMethodID(jniEnv->FindClass("java/lang/Class"), "getName", "()Ljava/lang/String;");
            ...
            LOG("superClass name = %s" , superClassNameStr);
            if(CD_NAME->empty()){
                jniEnv -> DeleteLocalRef(application_clz);
                return nullptr;
            }else{
                size_t size = sizeof(CD_NAME);
                for (size_t i = 0; i < size; ++i) {
                    if(CD_NAME[i] != superClassNameStr){
                        jniEnv -> DeleteLocalRef(application_clz);
                        return application;
                    }
                }
